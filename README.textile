...
p. When *Allow PHP in pages* option is disabled, @<txp:rah_function />@ tags can not be used within "page":http://textpattern.net/wiki/index.php?title=Pages and "form":http://textpattern.net/wiki/index.php?title=Forms templates. When *Allow PHP in articles* option is disabled, @<txp:rah_function />@ tags can not be used within articles. The tags won't be executed, no matter what permissions the article's author has.
...
p. Whitelisting options can be set from Textpattern's @config.php@ (e.g. /textpattern/config.php) file, and doesn't take more than adding a single line to it. Rah_function expects a constant named @rah_function_whitelist@, populated with comma-separated list of function names. Function names and class methods should be formatted in the same way as when used in rah_function tags @call@ attribute.
