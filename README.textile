...
p. The plugin introduces a new tag @<txp:rah_function />@ to Textpattern arsenal. It's a multi-purpose tag that enables use of public PHP functions and methods as Textpattern tags.

p. Rah_function tag is almost like a bridge between PHP and Textpattern's tag syntax. It allows calling almost any PHP function as a Textpattern tag. Want to encode something? You can. Want use PHP's string functions to truncate, count or otherwise bend strings at your will? You can. All without needing to write any code for new tags or add raw PHP blocks in page templates.

p. The tag takes @call@ attribute which sets the function that is called by the tag. All other attributes, apart from @thing@, used in the tag are passes to the called function as its arguments. The tag then processes the request and returns results.
...
p. A container translates seamlessly to a singular self-closing tag. The contained statement is just a function parameter as tag attributes, and as such, can be substituted with a tag attribute. Following does exactly same thing as the previous str_replace examples that used containers.
...
p. Both of the two following "htmlspecialchars":http://php.net/manual/en/function.htmlspecialchars.php uses do the same thing, but with different syntaxes. As a self-closing tag the sanitized string would be passed to the function as a tag attribute.

bc. <txp:rah_function call="htmlspecialchars" string="<p>Hello World!</p>" />
...
p. Returned "arrays":http://www.php.net/manual/en/language.types.array.php will be converted to "JSON":http://php.net/manual/en/function.json-encode.php representations. Rest of types including object, resource, NULL and callable will be discarded and a notice will be issued.
